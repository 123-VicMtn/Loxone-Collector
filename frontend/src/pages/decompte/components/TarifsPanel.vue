...
const props = defineProps<{ tarifs: Tarif[]; miniserver: string; readOnly?: boolean }>()
...
<template>
  <div class="overflow-x-auto">
    <table class="w-full border-collapse text-sm">
      <template v-if="tarifs.length">
        <thead>
          <tr class="border-b border-neutral-200 text-left text-neutral-500">
            ...
            <th v-if="!readOnly" class="py-2 pl-3"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="t in tarifs" :key="t.id" class="border-b border-neutral-100">
            ...
            <td v-if="!readOnly" class="py-2 pl-3">
      ...
      <tbody v-else>
        <tr>
          <td :colspan="readOnly ? 5 : 6" class="py-3 text-neutral-500">
            Aucun tarif enregistré : les colonnes HT / TVA / TTC du décompte restent vides.
            {{ readOnly ? 'Contactez un administrateur pour en configurer.' : "Tant qu'aucun prix n'est saisi ci-dessous." }}
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <p v-else class="mt-4 text-sm text-neutral-500">
    Lecture seule -- contactez un administrateur pour modifier les tarifs.
  </p>
</template>
